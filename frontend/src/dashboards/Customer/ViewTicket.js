  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 border rounded-lg shadow-lg bg-white">
      ...
      <div className="mt-4">
        ...
        <p><strong>Status:</strong> <span className={ticket.status === "Resolved" ? "text-green-600" : "text-red-600"}>{ticket.status}</span></p>
      </div>
      ...
      <div className="mt-6 border p-3 rounded-lg bg-gray-100">
        ...
        {ticket.responses && ticket.responses.length > 0 ? (
          ticket.responses.map((res, index) => (
            <p key={index} className={`mt-2 p-2 rounded-lg ${res.sender === "Customer" ? "bg-green-100" : "bg-gray-200"}`}>
