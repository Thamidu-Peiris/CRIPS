...
const growerHandlerModel = require("../../models/GrowerHandler/growerHandlerModel");
...
const getProfileById = async (req, res) => {
  try {
    const user = await growerHandlerModel.findById(req.params.id).select("-password");
    if (!user) {
...
const updateProfile = async (req, res) => {
  try {
    ...
    const updatedUser = await growerHandlerModel.findById(req.params.id);
    if (!updatedUser) {
      return res.status(404).json({ error: "User not found" });
    }
...
const changePassword = async (req, res) => {
  try {
    ...
    const user = await growerHandlerModel.findById(req.params.id);
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
...
const applyForJob = async (req, res) => {
  try {
    ...
    const existingUser = await growerHandlerModel.findOne({ email });
    if (existingUser) {
    ...
    const newApplication = new growerHandlerModel({
      firstName,
      lastName,
      username,
      address,
...
const login = async (req, res) => {
  try {
    ...
    const user = await growerHandlerModel.findOne({ email });
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
